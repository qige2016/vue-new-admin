...
├── styles
│   ├── common.scss              # 全局通用样式
│   ├── element.scss             # 全局自定义 element-ui 样式
│   ├── index.scss               # 全局引用
│   ├── mixin.scss               # 全局mixin
│   ├── reset.scss               # 全局重置样式
...
怎么覆盖 element-ui 样式？由于 element-ui 的样式是在全局引入的，所以想在某个页面里面覆盖它的样式就不能加 scoped，但如果只想覆盖这个页面的 element 样式，你就可在它的父级加一个 class，用命名空间来解决问题。
